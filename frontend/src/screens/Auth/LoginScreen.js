...
import { View, Text, StyleSheet, SafeAreaView, TouchableOpacity, Alert, TouchableWithoutFeedback, Keyboard } from 'react-native';
...
const LoginScreen = ({ navigation }) => {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      // Backend returns: data: { token: '...', roles: ['Admin'] }
      if (response && response.isSuccess) {
        const { token, roles } = response.data;
        // Assuming user has at least one role, pick the first one or prioritize Admin
        const role = roles.includes('Admin') ? 'Admin' : 'User';
        await signIn(token, role);
      } else {
        Alert.alert('Đăng nhập thất bại', response.message || 'Lỗi không xác định');
      }
    } catch (error) {
      Alert.alert('Lỗi', error.message || 'Sai tài khoản hoặc mật khẩu');
    } finally {
      setIsLoading(false);
    }
  };
